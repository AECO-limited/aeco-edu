...
const PartnerPathwayCard: React.FC<{ partner: Partner }> = ({ partner }) => {
    ...
    return (
        <div onMouseOver={() => { setHovered(true) }} onMouseLeave={() => { setHovered(false) }}
            ...
            <button className='text-white bg-[var(--secondary-500)] px-3 p-2 rounded-full  m-4'>{partner.name}</button>
            <div className="h-[60%] absolute bottom-0 overflow-hidden rounded-b-md  w-full p-3" style={{ backgroundImage: "linear-gradient(to top,  #0e4d4d, #0e4d4dee, #0e4d4dbb,transparent )" }}>
                <motion.section
                    initial={{ y: 250 }}
